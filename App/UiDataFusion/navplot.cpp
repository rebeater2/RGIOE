...
NavPlotErrorCode NavPlot::LoadFile_<NavAscii>(const std::string &file_path) {
    ...
    while (!ifs.eof()) {
        ...
        ss >> nav.week >> nav.gpst >> nav.lat >> nav.lon >> nav.height >> nav.vn[0] >> nav.vn[1] >> nav.vn[2]
           >> nav.atti[0]
...
NavPlotErrorCode NavPlot::LoadFile(const std::string &file_path, NavFileFormat fmt) {
    switch (fmt) {
        case NavFileFormat::NavBinary:
            return LoadFile_<NavFileFormat::NavBinary>(file_path);
        case NavFileFormat::NavAscii:
            return LoadFile_<NavFileFormat::NavAscii>(file_path);
        case NavFileFormat::NavDoubleMatrix:
            return LoadFile_<NavFileFormat::NavDoubleMatrix>(file_path);
        default:
            return NavPlotFileBadFormat;
    }
}
