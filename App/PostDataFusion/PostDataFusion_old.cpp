...
int main(int argc, char **argv) {
    ...
    std::shared_ptr<ImuData_t> imu = nullptr;
    std::shared_ptr<GnssData_t> gnss = nullptr;
    do {
        /*! step1: read raw data*/
...
//        LOG_IF(INFO, data->type == 2) << fmt::format("type {}:{:6f}", data->type, data->time);
        ...
        switch (data->type) {
            case DATA_TYPE_IMU: {
                imu = std::dynamic_pointer_cast<ImuData_t>(data);
            ...
            case DATA_TYPE_GNSS: {
                gnss = std::dynamic_pointer_cast<GnssData_t>(data);
        ...
        if (gnss and fabs(gnss->time - imu->time) < 0.6/opt.d_rate){
            auto rgioe_gnss = gnss->toRgioeData();
            rgioe_gnssupdate(rgioe_dev,imu->time,&rgioe_gnss);
        }
